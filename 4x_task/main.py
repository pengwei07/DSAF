...
def main():
    ...
    model1 = DSAF(in_channels=4, num_res_blocks=3, num_heads=1, patch_dim=1, growth_rate=16, num_dense_layers=3,
                in_channels1=8, num_res_blocks1=3, num_dense_layers1=3, growth_rate1=16, init_weights=init_weights1).to(device)

    model2 = DSAF(in_channels=4, num_res_blocks=3, num_heads=1, patch_dim=1, growth_rate=16, num_dense_layers=3,
    ...
    scheduler1 = optim.lr_scheduler.ReduceLROnPlateau(optimizer1, mode='min', factor=0.8, patience=5, verbose=True)
    scheduler2 = optim.lr_scheduler.ReduceLROnPlateau(optimizer2, mode='min', factor=0.8, patience=5, verbose=True)
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model1.state_dict(), '4x_model1.pth')
            torch.save(model2.state_dict(), '4x_model2.pth')
